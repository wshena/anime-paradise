...
const BannerInfo = ({title, image, rating, genre, synopsis, id}:BigBannerInfoProps) => {
  return (
    <div className="flex flex-col md:flex-row items-start w-full gap-3 md:gap-10 text-white">
      ...
      {/* anime informations */}
      <div className="flex flex-col items-start gap-3 md:gap-5 w-full md:w-[70%]">
        {/* title */}
        ...
        <div className="flex items-center gap-5">
          <Link href={`/anime/${id}/${createSlug(title)}`} aria-label='redirect to detail page'>
            <button className="flex items-center gap-4 bg-orange-500 py-2 px-4" aria-label='redirect to detail page'>
...
const AnimeBigBanner = ({item}:{item:any}) => {
  const coverImage = item && item.images?.jpg?.large_image_url;

  return (
    <div className="relative w-full h-[600px] lg:h-[600px] bg-repeat bg-contain" style={{
