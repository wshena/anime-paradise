...
export const metadata = {
  title: 'Browse Top Anime - Anime Paradise',
  description: "Embark on an anime adventure with Anime Paradise, your ultimate destination for watching a vast collection of anime series and movies. Delve into the captivating worlds of hit titles such as One Piece, Jujutsu Kaisen, Chainsaw Man, and Attack on Titan. Browse top anime list, find it, and watch it!",
}
    ...
    <MainWrapper>
      <ContentWrapper>
        <Stack gap={{base:'30px', md:'40px'}} paddingY={{ base: '30px', md: '50px' }}>
          <Heading as={'h1'} fontSize={{base:'1rem', md:'1.3rem', lg:'1.5rem'}}>Top Anime</Heading>
          {topAnimeList?.data?.length <= 0 ? (
            ...
            <>
              <Center width={'100%'}>
                ...
                >
                  {!topAnimeList ? (
                    ...
                    <>
                      {topAnimeList?.data?.map((item: any, idx:number) => (
              ...
              <Center width={'100%'}>
                <CustomPagination count={topAnimeList?.pagination?.items?.total} pageSize={topAnimeList?.pagination?.items?.per_page} defaultPage={1} />
              </Center>
            </>
          )}
        </Stack>
